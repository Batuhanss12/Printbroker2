...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/quotes', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const quoteData = {
        id: randomUUID(),
        customerId: userId, // This is the key field that was missing
        userId,
        title: title || `${type} Teklif Talebi`,
        type,
  ...
  app.post('/api/quotes', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Simplified quote creation without strict schema validation
      const quoteData = {
        id: randomUUID(),
        customerId: userId,
        title: req.body.title,
        type: req.body.type || 'general_printing',
      ...
      // Simulate quote distribution to printers
      console.log(`📢 Teklif ${quote.id} - ${quote.title} matbaa firmalarına iletildi`);
      
      res.json({
        success: true,
        message: "Teklif başarıyla gönderildi",
        quote: quote,
        estimatedResponse: "60 saniye içinde teklifler gelmeye başlayacak"
      });
    } catch (error) {
      console.error("Error creating quote:", error);
      res.status(500).json({ 
        success: false,
        message: 'Teklif gönderilirken hata oluştu. Lütfen tekrar deneyin.'
      });
    }
  });
